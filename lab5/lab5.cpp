...
void LinearLayer(float *A, float *B, float *C, float *D, int n, int k, int m) {
    ...
    {
        float sum = 0.0f;
        #pragma acc parallel loop reduction(+:sum) tile(32,32)//collapse(2) //reduction(+:sum)
        for (int i = 0; i < n; i++) {
...
void Sigmoid(float *A, int n, int m) {
    ...
    #pragma acc parallel loop tile(32,32) //collapse(2)
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            // Sigmoid(x) = 1/(1+exp(x))
...
/*
    #pragma acc data copyin(A[0:n*m]) copyout(D[0:n])//#pragma acc data copyin(A[0:n*m]) copyout(D[0:n])
    #pragma acc kernels//#pragma acc loop tile(32,2)
    {
        for (int i = 0; i < n; i++) {
            float mx = A[i * m];
            int index = 0;
